...
export default function Home() {
  ...
  const welcomePhrases = useMemo(
    () => [
      "Welcome to EatsAI — your NYC co-pilot.",
      "Drop in like Uber, chat like OpenAI, navigate like Citymapper.",
      "Find the spots locals whisper about while you’re still in the cab.",
    ],
    []
  );
  ...
  useEffect(() => {
    ...
    const atPhraseEnd = typedLine === phrase;

    const timeout = setTimeout(() => {
      if (typingForward) {
        if (typedLine.length < phrase.length) {
    ...
    }, typingForward && atPhraseEnd ? 1200 : typingForward ? 42 : 26);

    return () => clearTimeout(timeout);
  }, [introVisible, typedLine, typingForward, phraseIndex, welcomePhrases]);
          ...
          <div className="welcome-panel">
            <div className="flex flex-col gap-6 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm uppercase tracking-[0.35em] text-slate-300">EatsAI</p>
                  <p className="text-3xl md:text-4xl font-semibold">Arrival Intelligence</p>
                </div>
              </div>

              <div className="space-y-3">
                <p className="text-lg text-slate-200">An OpenAI-style welcome while we ready your city signals.</p>
                <p className="text-2xl md:text-3xl font-bold typing-line">{typedLine}</p>
                <p className="text-sm text-slate-300 max-w-3xl">
                  We prime your concierge, transit, and trend layers so you land with confidence. Tap begin to drop into the live map and chat experience.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-3 items-center">
                ...
                <span className="px-3 py-1 rounded-full bg-white/10 border border-white/10 text-sm">Live NYC signals warming up</span>
              </div>
            </div>
          </div>
        </div>
